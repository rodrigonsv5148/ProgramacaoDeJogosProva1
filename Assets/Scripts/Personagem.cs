...
public class Personagem : MonoBehaviour
{
    ...
    [SerializeField]private float tempoDeattack; 
    ...
    public Transform prefabSpecial;
    ...
    void OnAttack()
    {
        if(machinima == false)
        {
            if(Time.time > proximoAtaque && atacando == false)
            {
                atacando = true;
    ...
    void OnTriggerEnter2D(Collider2D other) 
    {
        if(machinima == false)
        {
            if(other.gameObject.CompareTag(nomeDaTag) && atacando == false)
            {
                hittedAudio.Play();
    ...
    // Funcao responsavel por dar um tempo de ataque
    private IEnumerator attackTime()
    {
        yield return new WaitForSeconds(tempoDeattack);
        atacando = false;
    }
    ...
    void SpecialAttack()
    {
        if(sr.flipX == false)
            {
                Transform novoProjetil = Instantiate(prefabSpecial, frente.position, Quaternion.identity);
                novoProjetil.GetComponent<Projetil>().direcao = Vector2.right;
                Destroy(novoProjetil.gameObject, 2.3f);

            }else
            {
                Transform novoProjetil = Instantiate(prefabSpecial, atras.position, Quaternion.identity);
                novoProjetil.GetComponent<Projetil>().direcao = Vector2.left;
                Destroy(novoProjetil.gameObject, 2.3f);
            }
    }
}
